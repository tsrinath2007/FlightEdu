  ...
  const syncSeatToDatabase = useCallback(async (seatNum: string, currentSubject?: string): Promise<{ ok: boolean; error?: string }> => {
    try {
      ...
      if (!res.ok) {
        ...
        const errMsg = errData.error || `Seat sync failed (${res.status})`;
        console.warn("Seat sync non-OK:", errMsg);
        return { ok: false, error: errMsg };
      }
      return { ok: true };
    } catch (e: any) {
      // Silently log — network errors, auth errors, should not crash the cockpit
      console.warn("Seat sync silently failed:", e?.message || e);
                 ...
                 <button
                  onClick={async () => {
                    ...
                    if (config) {
                      ...
                      const result = await syncSeatToDatabase(targetSeat as string, config.studySubject);
                      ...
                      if (!result.ok) {
                        ...
                        alert(result.error || "⚠️ This seat is already taken!");
                      } else {
                        if (soundEffectsEnabled) {
                          try {
                            const slide = new Audio("https://assets.mixkit.co/active_storage/sfx/2019/2019-84.wav");
